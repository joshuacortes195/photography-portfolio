...
export async function generateMetadata({
  ...
  if (!member) return { title: "Videography" };
  return {
    title: `${member.name} — Videography`,
    description: `Videography by ${member.name} at ${SITE.name}.`,
  };
}
...
export default async function VideographyPage({
  ...
  const videos = await listFolderMedia(member.drive.videos, { revalidate });
  ...
  return (
    ...
    >
      {videos.length > 0 ? (
        <Gallery items={videos} layout={getTheme(member.slug).layout} />
      ) : (
        <EmptyNote name={member.name} kind="videos" />
      )}
    </MemberShell>
  );
}
